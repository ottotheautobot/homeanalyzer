...
- **Camera-tracked floor plans (v2 of measurement).** v1 ships a Depth-Anything-V2-only pass (`modal_apps/floor_plan.py`) — per-frame metric depth aggregated by schematic time-window, no inter-frame tracking. Quality is rough (~30–50% dimension error, layout follows schematic adjacency). v2 should add MASt3R or VGGT-1B-Commercial for real camera tracking → globally consistent point cloud → polygon extraction (Mask2Former + RoomFormer or simpler shape-from-floor). Feasibility report in `docs/floorplan-slam-research.md`.
