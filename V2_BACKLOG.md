...
- **Measured 2D floor plans from tour video.** v1 ships a lo-fi schematic with Sonnet-estimated room dimensions (`backend/app/llm/floor_plan.py`); v2 needs real measurements. Detailed feasibility report in `docs/floorplan-slam-research.md`. Recommended pipeline: VGGT-1B-Commercial → Mask2Former → RoomFormer on Modal serverless H100. ~$0.60/tour compute, 6–10 engineer-weeks to ship behind a flag, ~70% first-shot success with a "slow 360° spin in each room" capture-protocol nudge. Crossover with CubiCasa API ($35–80/plan) is around 1k tours/month — until then, recommendation is to integrate CubiCasa or Polycam with a deliberate scan rather than build in-house.
