...
- **Bring streaming/recording fully into the app, replace the Zoom-bot middleman.** Today the host streams from Zoom's iPhone/desktop client and Meeting BaaS's bot relays the recording. Two big gains from owning the capture path: (1) **interactive capture protocol** — we can interject prompts ("hold the camera up here for 3 seconds", "you're going too fast through the kitchen") that improve floor-plan reconstruction quality dramatically, since today the user has no idea what the algorithm needs. (2) Eliminate Meeting BaaS as a dependency, vendor risk, and per-tour cost. Big architecture lift — would need to ship a real iOS client (already on backlog) or a PWA capture flow that holds the camera open + streams chunked video, plus the same for the partner's viewing experience that Zoom currently provides. Probably gates the v3 product story.
