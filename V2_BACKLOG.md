...
- **Floor-plan reconstruction polish.** v1.5 ships MASt3R sparse global alignment on Modal A10G (`modal_apps/floor_plan.py`) — produces unified-world-coord polygons with metric dimensions from the tour video. Gaps to address before this is "good": (a) build the curope CUDA kernel in the image so RoPE2D doesn't run pure-PyTorch (~3–5× speedup on the pairwise pass); (b) replace per-frame oriented-bbox-per-time-window with proper room segmentation — points leak between adjacent rooms through doorways, so e.g. a bathroom's points contain bedroom geometry; DBSCAN per room or a smarter view-association would fix it; (c) global Manhattan-axis alignment so all polygons share the same wall orientation rather than each having its own PCA axes; (d) RoomFormer or similar to snap to right-angled rooms when Manhattan prior holds. Feasibility detail in `docs/floorplan-slam-research.md`.
