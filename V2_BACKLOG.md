...
- **Measured 2D floor plans from tour video.** v1 ships a lo-fi schematic (room list + adjacencies + features, generated by Sonnet from the transcript/observations) — see `backend/app/llm/floor_plan.py`. v2 needs actual scaled wall geometry and dimensions. No commercial product currently does this from casual handheld tour video — all of CubiCasa/Polycam/Matterport assume deliberate capture. Paths to evaluate: (a) integrate a "spin the camera once per room" UX nudge to unlock per-room layout estimation via single-image models like RoomFormer/HorizonNet, (b) monocular video SLAM (SLAM3R, NeuralRecon) for trajectory + footprint — needs GPU infra, (c) DUSt3R-class 3D foundation models if/when they get a usable API, (d) optional CubiCasa API integration with a deliberate-capture step for users who want MLS-quality plans. **Real differentiator if cracked.**
